...
recomendar :-nl,
    write("Lo sentimos tu respuesta no es valida"),nl.
...
get_tipo :-
    ...
    (miembro(Tipo, Input), restaurante([_, Tipo, _, _, _]) ->
        nl,write("¿En que zona desea comer? "), nl,
        read(Provincia),
...
get_provincia(Input, Tipo) :-
    (restaurante([_, _, [Provincia|_], _, _]), sub_string(Input, _, _, _, Provincia) ->
        ...
        show_restaurantes(Tipo, Provincia),nl,
...
%funcion de conversion de string a lista de palabras
